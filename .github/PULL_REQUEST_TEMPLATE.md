## Summary
<!-- Single-line summary of the changes (e.g., Add new image filter node) -->

## Changes
<!-- Bulleted list of code/functional changes ONLY. -->
<!-- Do NOT include non-change items like "ran tests", "verified behavior", or "fixed linting". -->
<!-- Example:
- Added/updated Meld Image Manager components in `meld/image_manager/`
- Implemented Gaussian blur algorithm
-->

## Related Issues
<!-- List any related issues (e.g., #1) -->
