...
- **Image Processing Pipeline**: `meld/load_image_configs/core/image_processing.py` (`process_image_metadata()` — shared pipeline used by both `MeldImageLoader` and `MeldImageLoaderBatch`: metadata extraction → parameter cascade → checkpoint loading → output construction. Returns `ImageProcessingResult` TypedDict).
