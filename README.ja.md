...
- **データベース管理**: `Settings -> System` から複数データベースの作成・リネーム・切り替え・削除
...
Image Manager は **複数の SQLite データベース** にも対応しています。`Settings -> System` から管理でき、データベース切り替え時には Light Table やフロントキャッシュを明示的に初期化して、DB 間で状態が混ざらないようにしています。
