...
export const useWorkflowExecution = () => {
	const executeWorkflow = useCallback(
		async (
			...
			logger.log("[Meld] executeWorkflow called:", {
				workflowName,
				imageId: image.id,
				maskFilename,
				targetLoaderNodeId,
			...
			logger.log("[Meld] Workflow fetched:", workflowName);
			...
			logger.log("[Meld] Node IDs found:", {
				loaderNodeId,
				maskNodeId,
				isUIFormat,
			});
			...
			if (isUIFormat) {
				...
				logger.log("[Meld] Active graph nodes count:", activeNodes.length);
				...
				if (loaderNode) {
					...
					logger.log("[Meld] Updating loader node widget:", {
				...
				if (maskFilename) {
					...
					logger.log("[Meld] Updating mask node widget:", {
						nodeId: maskNode?.id,
						maskFilename,
					});
					if (maskNode) {
