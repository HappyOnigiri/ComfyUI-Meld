...
> => {
    const res = await api.fetchApi(`/meld-nexus/suggest-parents?id=${id}&threshold=${threshold}`);
    if (!res.ok) {
        return [];
    }
    return await res.json();
...
export const deleteImages = async (ids: number[], deleteFiles = true): Promise<void> => {
