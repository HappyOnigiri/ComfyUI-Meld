...
export const HistoryTreeModal: React.FC<HistoryTreeModalProps> = ({ imageId }) => {
    ...
    const renderNodes = (parentId: number | null, level = 0) => {
        ...
        return nodes.map((node) => (
            <div key={node.id} className="meld-tree-node-container">
                ...
                >
                    {level > 0 && <ChevronRight size={16} className="meld-tree-node-arrow" />}
                    <div className="meld-tree-node-content">
                        {/* biome-ignore lint/a11y/useKeyWithClickEvents: node preview */}
                        <img
                            ...
                            alt={node.filename}
                            onClick={() => dispatch({ type: "OPEN_VIEWER", payload: node.id })}
                        />
    ...
    const roots = lineage.filter((img) => !img.parent_id || !lineageIds.has(img.parent_id));
