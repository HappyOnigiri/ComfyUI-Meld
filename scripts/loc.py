...
def _linguist_generated_excluded(repo_root: Path, ls_files_z: bytes) -> set[str]:
    """Paths where `linguist-generated` is set (e.g. build artifacts in `.gitattributes`)."""
    if not ls_files_z:
        return set()
    attr = subprocess.run(
...
def main() -> int:
    ...
    print("Excluded paths are detected via `git check-attr linguist-generated` (see .gitattributes).")
    if excluded:
        print("")
