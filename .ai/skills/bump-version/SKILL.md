...
5.  **Rebuild and Commit Build Artifacts**:
    - Run `make ci` after updating the version file to regenerate build artifacts (e.g., `web/js/index.js`).
    - If `make ci` fails, stop the entire workflow and report the error to the user.
    - After `make ci` succeeds, check `git status` for any modified build artifacts.
    - If build artifacts have changed, stage them alongside `pyproject.toml` so they are included in the version bump commit.
    - **Reason**: The version string is embedded in the UI bundle at build time; committing stale artifacts causes CI to fail with a dirty working tree error.

6.  **Use the `ship` Skill**:
    ...
    - Invoke the **`ship`** skill to perform the following:
      ...
      - Commit the changed files (including any updated build artifacts) with the message: `chore: bump version to <version>`.
