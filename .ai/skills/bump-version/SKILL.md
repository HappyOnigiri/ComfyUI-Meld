...
5.  **Rebuild and Commit Build Artifacts**:
    ...
    - After `make ci` succeeds, stage the following known generated paths alongside `pyproject.toml` so they are included in the version bump commit (only if they exist and are modified):
      - `web/js/gallery_extension.js`
      - `web/js/style.css`
    - Do **not** stage other files under `web/` (e.g., `web/js/viewer.js`, `web/js/jszip.min.js`) as they are not regenerated by the build.
    - If the set of build outputs changes in the future, update this allowlist accordingly.
