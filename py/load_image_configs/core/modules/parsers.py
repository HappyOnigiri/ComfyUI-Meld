        ...
        try:
            ...
            for node in sorted_nodes:
                ...
                if n_type in subgraphs_dict:
                    ...
                    if res["has_sampler"]:
                        target_node = node
                        target_mapping = {"positive": res["positive_input"], "negative": res["negative_input"]}
    ...
    def parse_prompt_json(prompt_json: str | dict | None, logs: list[str]) -> tuple[str, str]:
        ...
        try:
            ...
            if sampler_node:
                            ...
                            for s_v in s_inputs.items():
                                if isinstance(s_v, list) and len(s_v) == 2 and str(s_v[0]) == str(node_id):
                                    for val_key in ["text_0", "text_1", "value"]:
                                        if (
                                            val_key in s_inputs
                                            and isinstance(s_inputs[val_key], str)
                                            and len(s_inputs[val_key]) > 3
                                        ):
                ...
                if negative:
                    logs.append(f"-> Negative found (API): {negative[:50]}{'...' if len(negative) > 50 else ''}")
            else:
        ...
        try:
            ...
            for node in sorted_nodes:
                ...
                if ctype in subgraphs_dict:
                    sub_params, sub_found = Parsers.get_ksampler_params(subgraphs_dict[ctype], logs, subgraphs_dict)
                    if sub_found:
            ...
            if target_node:
                ...
                if found:
                    logs.append(f"-> Found KSampler params in Node ID: {target_node.get('id')} ({ctype})")
            ...
            if sampler_node:
                ...
                if "scheduler" in inputs:
                    ...
                    if isinstance(val, str):
                        params["scheduler"] = val
                found = True
                logs.append(f"-> Found KSampler params in Prompt Node ID: {nid} ({sampler_node.get('class_type')})")
        except Exception as e:
                ...
                if ("Empty" in ctype and "Latent" in ctype) or "ImageScale" in ctype:
                    ...
                    if len(w_values) >= 2:
                        try:
                            ...
                            found = True
                            logs.append(f"-> Found Resolution in Node ID: {node.get('id')} ({ctype}: {width}x{height})")
                        except Exception:
                            continue
        except Exception as e:
