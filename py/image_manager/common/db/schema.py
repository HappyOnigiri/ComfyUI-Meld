...
def init_db(db_path: str | None = None, trash_dir: str | None = None) -> None:
    ...
    with db_connection(db_path) as conn:
        ...
        """)
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_analytics_resolution_count ON analytics_by_resolution(count DESC)"
        )
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_pp_rel_ppid ON positive_prompt_image_relations(positive_prompt_id)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_np_rel_npid ON negative_prompt_image_relations(negative_prompt_id)"
        )
